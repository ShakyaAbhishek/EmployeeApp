...
export default class WeeklySteps extends Component {
  constructor(props) {
    super(props);
    this.state = {
      stepDataArray: [500, 700, 400, 950, 800, 350, 530],
      weeklyStepData: [
        {
          weekDay: "Sunday",
          numberOfsteps: 1000
    ...
    GoogleFit.authorize(options)
      .then(res => {
        if (res.success) {
          ...
          const options = {
            startDate: d1, // required ISO8601Timestamp
            endDate: new Date().toISOString() // required ISO8601Timestamp
          };

          const option = {
            startDate: "2019-11-27T00:00:17.971Z", // required
          ...
          GoogleFit.getHeartRateSamples(option, callback)

          // let option = {
          //   startDate: new Date(2019, 9, 17).valueOf(), // simply outputs the number of milliseconds since the Unix Epoch
          //   endDate: new Date(2019, 9, 18).valueOf()
          // };
          // GoogleFit.getActivitySamples(option, (err, res) => {
  ...
  render() {
    ...
    const fill = "#4fb2a7";
    const data = [500, 700, 400, 950, 800, 350, 530];
    ...
    return (
      <View style={{ width: "100%", alignItems: "center" }}>
        ...
        >
          <View style={{ width: "80%" }}>
            {this.state.stepDataArray && this.state.stepDataArray.length == 7 ? (
              <StepBarGraph
                data={this.state.stepDataArray ? this.state.stepDataArray : data}
            ...
            return (
              ...
              >
                ...
                <View style={{ width: "33%", alignItems: "center" }}>
                  <Text style={{ color: "#fff" }}>{this.state.stepDataArray[index] ? this.state.stepDataArray[index] : steps.numberOfsteps}</Text>
                </View>
               {
                 this.state.stepDataArray[index] > 2000 ?  <View style={{ width: "33%", alignItems: "flex-end" }}>
                 <Image
                   source={Star}
